...
import { createDateFormatter, createFilter, useLocale } from "../i18n/index.jsx";
import { E as todayDate, T as toLocalISOString, _ as setMinutes, b as setYear, g as setHours, h as setDay, m as parseLocalISOString, n as compareDates, o as getFieldBounds, r as cycleField, v as setMonth, y as setSeconds } from "../date-math/eXQ3A5eo.jsx";
...
function DateFieldHiddenInput(props) {
	...
	return <input type="text" tabindex={-1} style={visuallyHiddenStyles} name={formControlContext.name()} value={serializedValue()} required={formControlContext.isRequired()} disabled={formControlContext.isDisabled()} readonly={formControlContext.isReadOnly()} aria-hidden="true" onChange={(e) => {
...
function DateFieldInput(props) {
	...
	return <Polymorphic as="div" role="presentation" ref={[dateFieldContext.setInputRef, mergedProps.ref]} aria-labelledby={dateFieldContext.fieldAriaLabelledBy()} aria-describedby={dateFieldContext.fieldAriaDescribedBy()} {...formControlContext.dataset()} {...others} onKeyDown={onKeyDown} onFocusOut={onFocusOut}>
...
function DateFieldValueDescription() {
	...
	return <Show when={isValid()}>
			<div id={defaultId} style={{ display: "none" }}>
				{context.translations().selectedDateDescription(context.formattedValue())}
			</div>
		</Show>;
}
//#endregion
	...
	*/
	...
	return <FormControlContext value={formControlContext}>
			<DateFieldContext value={context}>
				<Polymorphic as="div" {...formControlContext.dataset()} {...plainOthers()} ref={[setRef, mergedProps.ref]} role="group" id={access(mergedProps.id)} aria-invalid={formControlContext.validationState() === "invalid" ? "true" : void 0} aria-required={formControlContext.isRequired() ? "true" : void 0} aria-disabled={formControlContext.isDisabled() ? "true" : void 0} aria-readonly={formControlContext.isReadOnly() ? "true" : void 0} aria-labelledby={ariaLabelledBy()} aria-describedby={ariaDescribedBy()}>
...
function DateFieldSegment(props) {
	...
	return <Show when={mergedProps.segment.type !== "literal"} fallback={<Polymorphic as="div" aria-hidden={true} data-separator="" {...others}>
					{mergedProps.segment.text}
				</Polymorphic>}>
			<spin_button_exports.Root ref={[setRef, mergedProps.ref]} tabindex={formControlContext.isDisabled() ? void 0 : 0} value={mergedProps.segment.value} textValue={textValue()} minValue={mergedProps.segment.minValue} maxValue={mergedProps.segment.maxValue} validationState={formControlContext.validationState()} required={formControlContext.isRequired()} disabled={formControlContext.isDisabled()} readOnly={formControlContext.isReadOnly() || !mergedProps.segment.isEditable} contentEditable={isEditable()} inputMode={inputMode()} autocorrect={isEditable() ? "off" : void 0} autoCapitalize={isEditable() ? "off" : void 0} spellcheck={isEditable() ? false : void 0} enterkeyhint={isEditable() ? "next" : void 0} style={{ "caret-color": "transparent" }} aria-label={ariaLabel()} aria-labelledby={ariaLabelledBy()} aria-describedby={ariaDescribedBy()} data-placeholder={mergedProps.segment.isPlaceholder ? "" : void 0} data-type={TYPE_MAPPING[mergedProps.segment.type] || mergedProps.segment.type} {...formControlContext.dataset()} {...others} onKeyDown={onKeyDown} onBeforeInput={onBeforeInput} onInput={onInput} onFocus={onFocus} onIncrement={onIncrement} onDecrement={onDecrement} onIncrementPage={onIncrementPage} onDecrementPage={onDecrementPage} onDecrementToMin={onDecrementToMin} onIncrementToMax={onIncrementToMax} {...touchPropOverrides()}>
