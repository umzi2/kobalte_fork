...
function OTPFieldInput(props) {
	...
	return <>
			...
			<Polymorphic as="input" ref={[(el) => {
		setInputRef(el);
		el.value = context.value();
	}, mergedProps.ref]} onInput={onInput} onFocus={onFocus} onBlur={onBlur} onMouseOver={onMouseOver} onMouseLeave={onMouseLeave} onKeyDown={onKeyDown} onKeyUp={onKeyUp} inputMode="numeric" autocomplete="one-time-code" spellcheck={false} style={resolvedStyle()} pattern={patternRegex()?.source} data-kb-otp-field-input="" disabled={formControlContext.isDisabled()} readonly={formControlContext.isReadOnly()} aria-required={formControlContext.isRequired() ? "true" : void 0} aria-disabled={formControlContext.isDisabled() ? "true" : void 0} aria-readonly={formControlContext.isReadOnly() ? "true" : void 0} aria-invalid={formControlContext.validationState() === "invalid" ? "true" : void 0} {...others} />
		</>;
}
//#endregion
...
function OTPFieldRoot(props) {
	...
	return <FormControlContext value={formControlContext}>
			<OTPFieldContext value={context}>
				<Polymorphic as="div" ref={[setRef, mergedProps.ref]} style={resolvedStyle()} role="group" id={access(mergedProps.id)} data-kb-otp-field-root="" {...formControlContext.dataset()} {...others} />
			</OTPFieldContext>
		</FormControlContext>;
}
//#endregion
