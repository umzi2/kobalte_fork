...
import { createNumberFormatter, useLocale } from "../i18n/index.jsx";
...
import { a as stopEventDefaultAndPropagation, i as linearScale, n as getNextSortedValues, r as hasMinStepsBetweenValues, t as getClosestValueIndex } from "../utils/9uVKQNrd.jsx";
...
function SliderFill(props) {
	...
	return <Polymorphic as="div" style={combineStyle({
		[context.startEdge()]: `${offsetStart()}%`,
		[context.endEdge()]: `${offsetEnd()}%`
	}, props.style)} {...context.dataset()} {...others} />;
}
//#endregion
...
function SliderThumb(props) {
	...
	return <ThumbContext value={{ index }}>
			<Polymorphic as="span" ref={[setRef, mergedProps.ref]} role="slider" id={fieldProps.id()} tabindex={context.state.isDisabled() ? void 0 : 0} style={combineStyle({
...
function SliderInput(props) {
	...
	return <input type="range" id={fieldProps.id()} name={formControlContext.name()} tabindex={context.state.isDisabled() ? void 0 : -1} min={context.state.getThumbMinValue(thumb.index())} max={context.state.getThumbMaxValue(thumb.index())} step={context.state.step()} value={context.state.values()[thumb.index()]} required={formControlContext.isRequired()} disabled={formControlContext.isDisabled()} readonly={formControlContext.isReadOnly()} style={combineStyle({ ...visuallyHiddenStyles }, mergedProps.style)} aria-orientation={context.state.orientation()} aria-valuetext={valueText()} aria-label={fieldProps.ariaLabel()} aria-labelledby={fieldProps.ariaLabelledBy()} aria-describedby={fieldProps.ariaDescribedBy()} aria-invalid={formControlContext.validationState() === "invalid" ? "true" : void 0} aria-disabled={formControlContext.isDisabled() ? "true" : void 0} aria-readonly={formControlContext.isReadOnly() ? "true" : void 0} onChange={onChange} {...context.dataset()} {...others} />;
}
//#endregion
	...
	return <DomCollectionProvider>
			<FormControlContext value={formControlContext}>
				<SliderContext value={context}>
					<Polymorphic as="div" ref={[setRef, mergedProps.ref]} role="group" id={access(mergedProps.id)} {...dataset()} {...others} />
				</SliderContext>
			</FormControlContext>
...
function SliderTrack(props) {
	...
	return <Polymorphic as="div" ref={[context.registerTrack, props.ref]} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} {...context.dataset()} {...others} />;
}
//#endregion
