...
import { i as linearScale } from "../utils/9uVKQNrd.jsx";
...
function ColorAreaBackground(props) {
	...
	return <Polymorphic as="div" ref={[context.setBackgroundRef, props.ref]} style={combineStyle({
		"touch-action": "none",
...
function ColorAreaHiddenInputBase(props) {
	...
	return <input type="range" id={fieldProps.id()} name={(isVertical() ? context.yName() : context.xName()) || formControlContext.name()} tabindex={context.state.isDisabled() ? void 0 : -1} min={isVertical() ? context.state.yMinValue() : context.state.xMinValue()} max={isVertical() ? context.state.yMaxValue() : context.state.xMaxValue()} step={isVertical() ? context.state.yStep() : context.state.xStep()} value={isVertical() ? context.state.yValue() : context.state.xValue()} disabled={formControlContext.isDisabled()} readonly={formControlContext.isReadOnly()} style={combineStyle({ ...visuallyHiddenStyles }, mergedProps.style)} aria-roledescription={context.translations().twoDimensionalSlider} aria-valuetext={valueText()} aria-orientation={mergedProps.orientation} aria-label={ariaLabel()} aria-labelledby={fieldProps.ariaLabelledBy()} aria-describedby={fieldProps.ariaDescribedBy()} aria-invalid={formControlContext.validationState() === "invalid" ? "true" : void 0} aria-disabled={formControlContext.isDisabled() ? "true" : void 0} aria-readonly={formControlContext.isReadOnly() ? "true" : void 0} data-orientation={mergedProps.orientation} onChange={onChange} {...formControlContext.dataset()} {...others} />;
}
//#endregion
...
function ColorAreaThumb(props) {
	...
	return <Polymorphic as="span" ref={[context.setThumbRef, props.ref]} role="presentation" tabindex={context.state.isDisabled() ? void 0 : 0} style={combineStyle({
		position: "absolute",
	...
	}, props.style)} aria-label={ariaLabel()} onKeyDown={onKeyDown} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} {...formControlContext.dataset()} {...others} />;
}
//#endregion
