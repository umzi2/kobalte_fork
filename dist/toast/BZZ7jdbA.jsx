...
import { t as DATA_TOP_LAYER_ATTR } from "../layer-stack/DUp9FXTP.jsx";
...
function ToastCloseButton(props) {
	...
	return <button_exports.Root aria-label={props["aria-label"] || context.translations().close} onClick={onClick} {...others} />;
}
//#endregion
...
function ToastDescription(props) {
	...
	return <Polymorphic as="div" id={mergedProps.id} {...others} />;
}
//#endregion
...
function ToastList(props) {
	...
	return <Polymorphic as="ol" ref={[setRef, props.ref]} tabindex={-1} onFocusIn={onFocusIn} onFocusOut={onFocusOut} onPointerMove={onPointerMove} onPointerLeave={onPointerLeave} {...others}>
...
function ToastProgressFill(props) {
	...
	return <Polymorphic as="div" style={combineStyle({ "--kb-toast-progress-fill-width": `${lifeTime()}%` }, props.style)} {...others} />;
}
//#endregion
...
function ToastRegion(props) {
	...
	return <ToastRegionContext value={context}>
			<Polymorphic as="div" role="region" tabindex={-1} aria-label={ariaLabel()} style={combineStyle({ "pointer-events": hasToasts() ? mergedProps.topLayer ? "auto" : void 0 : "none" }, mergedProps.style)} {...topLayerAttr()} {...others} />
		</ToastRegionContext>;
}
//#endregion
...
function ToastRoot(props) {
	...
	return <Show when={present()}>
			<ToastContext value={context}>
				<Polymorphic as="li" ref={[setRef, mergedProps.ref]} role="status" tabindex={0} style={combineStyle({
		animation: isAnimationEnabled() ? void 0 : "none",
		"user-select": "none",
		"touch-action": "none"
	}, mergedProps.style)} aria-live={mergedProps.priority === "high" ? "assertive" : "polite"} aria-atomic="true" aria-labelledby={titleId()} aria-describedby={descriptionId()} data-opened={isOpen() ? "" : void 0} data-closed={!isOpen() ? "" : void 0} data-swipe-direction={rootContext.swipeDirection()} onKeyDown={onKeyDown} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} {...others} />
			</ToastContext>
		</Show>;
}
