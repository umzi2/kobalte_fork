...
import { createComponent, effect, mergeProps, setAttribute, template } from "@solidjs/web";
...
function PopperArrow(props) {
	...
	return createComponent(Polymorphic, mergeProps({
		...
		effect(() => rotate(), (_v$) => {
			setAttribute(_el$2, "transform", _v$);
		});
		return _el$;
	} }));
}
...
function PopperPositioner(props) {
	...
	return createComponent(Polymorphic, mergeProps({
		as: "div",
		ref: [(el) => { context.setPositionerRef(el); queueMicrotask(() => context.startUpdates?.()); }, props.ref],
...
function PopperRoot(props) {
	...
	return createComponent(PopperContext, {
		value: {
			currentPlacement,
